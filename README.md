...
3. 初回のみ、ターミナルで以下を実行（quarantine 属性を外す）:
   ```sh
   xattr -dr com.apple.quarantine /Applications/Yomikura.app
   ```
4. Launchpad / Spotlight から `Yomikura` を起動

> Apple Developer Program に未加入のため、macOS Big Sur 以降では `.dmg` から取り出した直後の `.app` に「壊れているため開けません」エラーが出ます。これはダウンロード時に付与される `com.apple.quarantine` 拡張属性が原因で、上記の `xattr -dr` で外せます（自分でビルド元を確認できるアプリなので問題ありません）。
>
> 「右クリック → 開く」だけでは現代の macOS では回避できないため、上記コマンドが必要になります。
